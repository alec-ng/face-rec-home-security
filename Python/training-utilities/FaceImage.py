...
class FaceImage:
    ...
    def cropToFace(self):
        ...
        # Scale image
        scImg = cv2.resize(self.image, (scSize.w, scSize.h), interpolation=cv2.INTER_LANCZOS4)
        ...
        if eyeAngle == 0:
            ...
            rotMatrix = cv2.getRotationMatrix2D((MID_X_TARGET, MID_Y_TARGET), eyeAngle, 1)
            self._finalImg = cv2.warpAffine(translatedScaledImage, rotMatrix, (WIDTH_TARGET, HEIGHT_TARGET))
    ...
    def save(self, outputpath):
        ...
        cv2.imwrite(outputpath, self._finalImg)

    def _getEyePair(self):
        cascade = cv2.CascadeClassifier(HC_EYEPAIRPATH)
    ...
    def _getEyes(self, eyepair):
        lEyeCascade = cv2.CascadeClassifier(HC_LEFTEYEPATH)
        rEyeCascade = cv2.CascadeClassifier(HC_RIGHTEYEPATH)
    ...
    def _getFace(self):
        ...
        cascade = cv2.CascadeClassifier(HC_FACEPATH)
    ...
    def _markRect(self, rect, color):
        ...
        cv2.rectangle(self.image, (rect.x, rect.y), (rect.x + rect.w, rect.y + rect.h), color)
...
def crop(image, offset, size):
    ...
    if offset.x > 0 or \
       ...
       offset.y + imageSize.h < size.h:
        ...
        finalImg = cv2.copyMakeBorder(image, offsTop, offsBottom, offsLeft, offsRight, GAP_BORDER)
